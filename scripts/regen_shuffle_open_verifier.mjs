// Regenerates the deployed MERGED SHUFFLE+OPEN verifier's proving artifacts.
//
...
//   node scripts/regen_shuffle_open_verifier.mjs <prover.toml>
...
const BUILD = join(root, 'circuits/shuffle_open_verifier/example_proof/beta16_build');
const OUT = join(root, 'circuits/shuffle_open_verifier/example_proof');
...
const circuit = JSON.parse(readFileSync(join(BUILD, 'target/shuffle_open.json'), 'utf8'));
...
console.log('\nNow:');
console.log('  garaga gen --system ultra_keccak_zk_honk --vk circuits/shuffle_open_verifier/example_proof/vk.bin --project-name shuffle_open_verifier');
