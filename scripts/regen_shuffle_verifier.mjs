// Regenerates the deployed shuffle verifier's proving artifacts.
//
...
//   node scripts/regen_shuffle_verifier.mjs <prover.toml>
...
const BUILD = join(root, 'circuits/shuffle_verifier/example_proof/beta16_build');
const OUT = join(root, 'circuits/shuffle_verifier/example_proof');
...
const circuit = JSON.parse(readFileSync(join(BUILD, 'target/shuffle.json'), 'utf8'));
...
console.log('\nNow:');
console.log('  garaga gen --system ultra_keccak_zk_honk --vk circuits/shuffle_verifier/example_proof/vk.bin --project-name shuffle_verifier');
